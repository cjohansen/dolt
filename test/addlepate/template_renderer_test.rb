...
describe Addlepate::TemplateRenderer do
  it "reads template from file" do
    File.expects(:read).with("/addlepate/views/file.erb").returns("")
    renderer = Addlepate::TemplateRenderer.new("/addlepate/views")
  ...
  it "does not cache template in memory when configured not to" do
    renderer = Addlepate::TemplateRenderer.new("/addlepate/views", :cache => false)
    File.stubs(:read).returns("Original")
    renderer.render(:file)
    File.stubs(:read).returns("Updated")
  ...
  it "renders template with layout" do
    renderer = Addlepate::TemplateRenderer.new("/", :layout => "layout")
  ...
  it "renders str templates" do
    renderer = Addlepate::TemplateRenderer.new("/", :type => :str)
  ...
  it "renders with helper module" do
    renderer = Addlepate::TemplateRenderer.new("/")
